...
TaskManager.defineTask(BACKGROUND_TASK_NAME, async () => {
  try {
    logger.log('[BackgroundTask] Running daily quote rotation task');
    const quote = await rotateDailyQuoteInBackground();

    if (!quote) {
      logger.log('[BackgroundTask] No quotes available');
      return BackgroundTask.BackgroundTaskResult.Success;
    }

    logger.log('[BackgroundTask] Quote rotated to:', quote.id, quote.text.substring(0, 30));

    const isDark = await getDarkBackground();
    const cachedPath = getCachedWallpaperPath(quote.id, isDark);

    if (!cachedPath) {
      logger.log('[BackgroundTask] No cached wallpaper for quote', quote.id, '- user needs to open app');
      return BackgroundTask.BackgroundTaskResult.Success;
    }

    const result = await setBothWallpapers(cachedPath);
    if (result.success) {
      logger.log('[BackgroundTask] Wallpaper set successfully for quote', quote.id);
    } else {
      logger.log('[BackgroundTask] Failed to set wallpaper:', result.error);
    }

    return BackgroundTask.BackgroundTaskResult.Success;
  } catch (error) {
    logger.error('[BackgroundTask] Error:', error);
...
export async function registerBackgroundTask(): Promise<boolean> {
  try {
    ...
    if (status === BackgroundTask.BackgroundTaskStatus.Restricted) {
      logger.log('[BackgroundTask] Background tasks are not available');
      return false;
    }
    ...
    logger.error('[BackgroundTask] Registration error:', error);
    return false;
  }
}
...
export async function unregisterBackgroundTask(): Promise<void> {
  try {
    ...
    logger.error('[BackgroundTask] Unregister error:', error);
  }
}
...
export async function getBackgroundTaskStatus(): Promise<{
  ...
  try {
    ...
    logger.error('[BackgroundTask] Status check error:', error);
